...
def history():
    if "user" in session:
        ...
        if user:
            ...
            purchases_with_products = [(purchase, Product.query.get(purchase.product_id)) for purchase in purchases]
            return render_template("order_history.html", user=username, signed=True, purchases=purchases_with_products, admin=user.admin)
    return redirect("/")
